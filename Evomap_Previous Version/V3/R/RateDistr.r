...
#' @param Y AncObject of variable 1
#' @param X AncObject of variable 2
#' @return list of ratedistribution elements and branches for each quadrant
#' @export

RateDistr<-function(Y,X,neutral){
            #relative rate
                    YvX<-Y[1:3]
                    D<-Y$rate-X$rate
      ...
      #Return
            Output<-list(YvX,S1,AI,DI,S2,AD,DD,all)
            names(Output)<-c("YvX","S1","AI","DI","S2","AD","DD","All")
            return(Output)
