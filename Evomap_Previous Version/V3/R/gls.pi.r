...
#predint
      SEYhat<-sqrt((diag(XX%*%C%*%t(XX))+as.matrix(1/k))%*%Y_MSEresid)
...
#confint extrapolated
      ...
      SEYhat.Xi<-sqrt((diag(ZZ%*%C%*%t(ZZ))+as.matrix(1/k))%*%Y_MSEresid)
      Yhat.Xi<-a+b*Xi
...
Output<-list(model,PI,PI.plot)
names(Output)<-c("model","PI","PI.plot")
return(Output)
}
